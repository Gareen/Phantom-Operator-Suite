...
import (
	...
	"Phantom-Operator/cmd/server"
	"Phantom-Operator/pkg/colors"
...
var (
	...
	Phantom-OperatorCli = &cobra.Command{
		Use:          "havoc",
		Short:        fmt.Sprintf("Phantom-Operator Framework [Version: %v] [CodeName: %v]", VersionNumber, VersionName),
		SilenceUsage: true,
		RunE:         teamserverFunc,
	}
...
// init all flags
func init() {
	Phantom-OperatorCli.CompletionOptions.DisableDefaultCmd = true
	...
	Phantom-OperatorCli.Flags().SortFlags = false
	Phantom-OperatorCli.AddCommand(CobraServer)
	Phantom-OperatorCli.AddCommand(CobraClient)
}

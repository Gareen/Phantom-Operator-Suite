# Phantom-Operator Teamserver

Source code of Phantom-Operator teamserver. Written in Golang.
...
- **Pre-requisites**
	1. Go1.18
- **Native**
	- To build the Teamserver client locally, run the following command in this folder(`~/Phantom-Operator/Teamserver/`):
		1. `make`
...
- **Jenkins Docker**
	- We can also build the Teamserver binary using a pre-configured Jenkins Docker image.
		1. From the parent folder(`Phantom-Operator`), run the following command to build the container:
		...
		3. We can now visit Jenkins at `localhost:8080` and create a Pipeline to build the Phantom-Operator Teamserver!
			* For a pre-done Groovy script, please see the `Phantom-Operator-Teamserver.groovy` in the `Assets` folder.
